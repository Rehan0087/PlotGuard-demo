...
cmd_start() {
  ...
  if podman ps --filter "name=$DB_CONTAINER" --format '{{.Names}}' 2>/dev/null | grep -q "$DB_CONTAINER"; then
    ...
    echo "  ! no container named '$DB_CONTAINER' found — create it first, this script only starts it"
    exit 1
  fi
  if wait_for_db; then
...
cmd_stop() {
  ...
  if [ "${1:-}" = "--with-db" ]; then
    ...
    if podman stop "$DB_CONTAINER" >/dev/null 2>&1; then
...
cmd_status() {
  echo "== Database =="
  local db_status
  db_status="$(podman ps --filter "name=$DB_CONTAINER" --format '{{.Status}}' 2>/dev/null)"
  if [ -n "$db_status" ]; then
    echo "  up   $db_status"
  else
    echo "  down"
